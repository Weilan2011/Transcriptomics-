...
for filename in os.listdir(directory):
    if filename.endswith(".MATS.JCEC.txt"):
        ...
        df_filtered = df[df['FDR'] < 0.05].copy()

        # Rename FDR and IncLevelDifference columns to include the filename or a unique identifier
        base_filename = filename.replace(".MATS.JCEC.txt", "")
        df_filtered.rename(columns={
            "FDR": f"{base_filename}_FDR", 
        ...
        if as_type not in combined_data:
            ...
            combined_data[as_type] = pd.merge(
                combined_data[as_type], df_filtered, 
                on=names[:-2],  # Assuming the last two names are FDR and IncLevelDifference
                how='outer'
            )
...
for as_type, df in combined_data.items():
    ...
    output_filename = f"combined_{as_type}.csv"
